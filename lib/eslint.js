...
function run(console, runOptions) {
  ...
  if(debug) {
    console.error("Blocked rules turned off:", patch.disabledRules)
    console.error("Blocked settings removed:", patch.removedSettings)
    console.error("SKIPPED MODULES", patch.skippedModules)
    console.error("LOADED MODULES", patch.loadedModules)
  }
}
